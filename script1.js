    ...
    function generateLinkHtml(url) {
        ...
        if (ytMatch && ytMatch[1]) {
            ...
            <div class="w-full aspect-video rounded-xl overflow-hidden mt-2 mb-3 shadow-sm relative group bg-cover bg-center" style="background-image: url('https://img.youtube.com/vi/${ytId}/hqdefault.jpg');">
                ...
                <div class="absolute top-0 left-0 right-0 h-[80%] z-20 cursor-pointer" onclick="const ifr=this.previousElementSibling; ifr.isPaused=!ifr.isPaused; ifr.contentWindow.postMessage(JSON.stringify({event: 'command', func: ifr.isPaused ? 'pauseVideo' : 'playVideo', args: []}), '*');"></div>
            </div>`;
        }
