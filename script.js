...
async function renderReels(container) {
    ...
    try {
        ...
        container.innerHTML = `<div class="reels-wrapper">
            ${videoPosts.map(p => {
                const videoUrl = p.video || p.image;
                ...
                <div class="reel-card" id="reel-${p._id}">
                    ...
                    <video loop muted playsinline webkit-playsinline preload="auto" 
                        class="reel-video opacity-0 transition-opacity duration-500" 
                        id="vid-${p._id}"
                        ...
                        ontimeupdate="updateReelProgress('${p._id}')"
                        onclick="handleReelClick(event, '${p._id}')">
                    ...
                    <div class="reel-actions-overlay">
                        ...
                        <div class="flex flex-col items-center" onclick="downloadReelWithProgress('${videoUrl}', '${p._id}')">
                            <i class="fa-solid fa-download text-2xl text-white" id="dl-icon-${p._id}"></i>
            ...
            entries.forEach(entry => {
                ...
                if (v) {
                    if (entry.isIntersecting) {
                        ...
                        updateMuteUI(v.id.split('-')[1], v.muted);
                    } else {
                        v.pause();
                        v.currentTime = 0;
                    }
