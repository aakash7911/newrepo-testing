    ...
    try {
        ...
        container.innerHTML = `<div class="reels-wrapper">
            ${videoPosts.map(p => {
                ...
                // 🔥 SUPER-FAST STREAMING PATCH (Frontend Layer) 🔥
                if (isYouTube) {
                    if (!videoUrl.includes('enablejsapi=1')) {
                        const separator = videoUrl.includes('?') ? '&' : '?';
                        videoUrl += `${separator}autoplay=1&mute=1&controls=0&modestbranding=1&rel=0&showinfo=0&iv_load_policy=3&disablekb=1&playsinline=1&enablejsapi=1&preload=auto&vq=medium`;
                    }
                }
