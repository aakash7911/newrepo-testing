...
function trackCategoryFromPost(postId) {
    ...
    function openLink(url) {
        ...
        setTimeout(() => {
            ...
            if (modal.children.length > 0) modal.children[0].classList.remove('scale-95');
        }, 10);
    }
...
async function renderChat(c) {
    c.innerHTML = `
    <div class="glass-card flex flex-col overflow-hidden p-0" style="position: fixed; top: 60px; bottom: 64px; left: 50%; transform: translateX(-50%); width: calc(100% - 16px); max-width: 432px; z-index: 10;">
...
function drawClassesUI() {
    ...
    let sortedLinks = [...classesToShow].filter(p => {
        ...
        const isVideo = (p.video) || 
                        (p.category === 'youtube_reel' || p.category === 'reel') || 
                        ['.mp4', '.webm', '.mov', '.ogg', 'youtube.com', 'youtu.be'].some(str => urlStr.includes(str));
        const hasLink = (p.link && p.link !== 'null' && p.link !== 'undefined') || (decContent && decContent.toLowerCase().includes('http'));
